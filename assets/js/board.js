...
function buildSpotCard(spot) {
  return `
    <a href="spot.html?id=${spot.id}" class="block rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition duration-300 bg-white">
...
async function loadSpots() {
  ...
  <a href="spot.html?id=${spot.id}" class="block min-w-[240px] rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition duration-300 bg-white">
    <img src="${spot.image}" alt="${spot.title}" class="w-full h-40 object-cover">
    <div class="p-4">
      <h3 class="font-bold text-slate-900 mb-2">${spot.title}</h3>
      <p class="text-xs text-slate-500 mb-3 line-clamp-2">${spot.address}</p>
